...
interface OwnProps {
  ...
  updateRoutes: (routes: InboundVoiceRoute[]) => void;
}

const RouteDataTable = (props: OwnProps) => {
  const [ showPanel, setShowPanel ] = useState(false);
  const [ selectedRoute, setSelectedRoute ] = useState(null as InboundVoiceRoute | null);
  ...
  const onRowClick = (item: InboundVoiceRoute) => {
    setSelectedRoute(item);
  }
  
  const onUpdateRoute = (newRoutes: InboundVoiceRoute[]) => {
      ...
      <div>
        ...
        <DataTable
          ...
          <ColumnDefinition
            ...
            header="Address"
            sortDirection='asc'
            sortingFn={(a: InboundVoiceRoute, b: InboundVoiceRoute) => (a.address > b.address) ? 1 : -1}
